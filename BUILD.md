...
| 5. x402 Payment Protocol | ✅ Done | `@x402/core@2.10.0`, `@x402/evm@2.10.0`, `@x402/fetch@2.10.0`, `@x402/express@2.10.0` + `viem@2.48.4` installed. Implemented against the **real v2 API** (BUILD.md spec referenced outdated v1 names). `protocol/x402/client.ts` uses `wrapFetchWithPaymentFromConfig` + `ExactEvmScheme` (viem signer); `protocol/x402/middleware.ts` uses `paymentMiddleware` + `x402ResourceServer` + `HTTPFacilitatorClient` against `https://facilitator.x402.org`. Headers: `PAYMENT-SIGNATURE` / `PAYMENT-REQUIRED` / `PAYMENT-RESPONSE`. `scripts/verify-x402.ts` passes; full `tsc --noEmit` clean. |
| 6. KeeperHub Automation | ✅ Done (code + live setup) / ⏳ Workflow deferred to §8 | `kh` CLI v0.9.0 + `KH_API_KEY` configured. `keepers/jobs/pushExpire.ts` + `keepers/workflows/order-monitor.yaml` in place; `scripts/verify-keeperhub.ts` passes. **Live:** ✅ KeeperHub MCP registered + authenticated via `/mcp` OAuth (`keeperhub: https://app.keeperhub.com/mcp` connected); ✅ agentic wallet provisioned via `keeperhub-wallet add` → subOrgId `caf8dd97-b006-473f-8040-d033dbb9eb9d`, walletAddress `0x7125054144A4B784D074Ef1294fc204aCa606936`, config at `~/.keeperhub/wallet.json`. **§6 monitor workflow (`tej561683650z2w8d1q1b`) wired live on KeeperHub:** Block(10)-trigger on chain 84532 → web3/read-contract `getLockedOrders()` on Base Sepolia Escrow `0x42A50591…3e1` → Condition(length>0) → For Each → web3/read-contract `getOrderDeadline(currentItem)` → Condition(deadline < `Math.floor(Date.now()/1000)`) → web3/write-contract `expire(orderId)` → discord/send-message. JSON pushed via `kh workflow update`. Pending: `kh wf go-live tej561683650z2w8d1q1b` and (optionally) configuring a real Discord integration. |
| 7. Payment Verification (KeeperHub Webhooks) | ✅ Done — full live e2e through KeeperHub | `hono@4` + `@hono/node-server` installed. `agents/payment-verify/{webhook.ts,banksim.ts,workflow.yaml}` in place. `webhook.ts` exports `verifyWebhookSignature` (timing-safe HMAC-SHA256), `validatePayment` (amount/currency/receiver/timestamp ±10min), `pinEvidence` (0G Storage upload via existing `ZeroGStorage`), `releaseEscrow` (no-op when `ESCROW_ADDRESS` unset), and `createWebhookServer` (Hono on `:4001` with `skipPin`/`skipRelease` flags for offline tests). `scripts/verify-section7.ts` boots a live receiver on `:4099` and proves: tampered body/sig rejected, mismatched amount/receiver/timestamp rejected, happy-path pinned to 0G Storage live (`0x89b022f6…d80b`, root `0x366d35…f8fc`, txSeq 49031); tampered sig → 401; unknown orderId → 404. **FULL E2E THROUGH KEEPERHUB:** `PSP (BankSim) → KH Webhook trigger (wfb_ key auth) → KH Code action (pure-JS HMAC-SHA256 verifies inline _signature field, since KH sandbox has no node:crypto / no Web Crypto) → KH Send Webhook → cloudflared tunnel → local receiver → 0G Storage pin → Base Sepolia Escrow.release()`. Last live run: KH execution `edlm48vm0flt0o73yn79h`, on-chain tx `0x335f7fa891ca6171f506de68ebdf26f2bcafd5a79d967352de390ee7fea79c34`, evidenceHash `0xe9c7617e…64cc`, Released event observed. **Bug found + fixed:** webhook.ts split `ESCROW_RPC_URL` (Base Sepolia) from `ZEROG_TESTNET_RPC` (always Galileo) so the 0G Storage SDK doesn't accidentally call Galileo-only `flow.market()` on the wrong chain. KH workflow `kap46a2jtab0dex082u0y` is enabled; receiver runs via `scripts/run-webhook-receiver.ts`; tunnel via `cloudflared tunnel --url http://localhost:4001`. |
| 8. Smart Contracts | ✅ Done (deployed) / ⏳ Post-deploy follow-ups | Foundry project initialised in `contracts/` with OZ v5.6.1 + forge-std v1.16. `foundry.toml` set to `via_ir=true` + `evm_version=cancun` + optimizer (resolves Stack-too-deep on Escrow). Four contracts written and deployed live to 0G Galileo (chain 16602) by `forge script Deploy.s.sol --slow --priority-gas-price 2000000000`: `BankSimVerifier=0xeec45fd463ea8137e46170694414703ebb791119`, `RailRegistry=0x8c7ffc95fcd2b9dfb48272a0ceb6f54e7ce77b14` (`banksim` rail registered), `AgentRegistry=0x98efa762eda5fb0c3ba02296c583a5a542c66c8b`, `Escrow=0x31da867c6c12ecebbb738d97198792901431e228` (deployer = keeper). Bytecode confirmed via `eth_getCode` on all four. Addresses written to `.env`. **Escrow has dual release paths:** legacy `release(uint256)` (zkTLS via verifier) and webhook `release(bytes32 orderIdHash, bytes32 evidenceHash)` keeper-only — matches §7's `releaseEscrow` ABI. **Mirror on Base Sepolia (84532) for KeeperHub integration:** `Escrow=0x42A50591420D4f623706816C8d7fE3B1Ee37B3e1`, `RailRegistry=0x8E55f999…cbD20`, `AgentRegistry=0xf03F328b…1064d`, `BankSimVerifier=0x8AD91327…2a82d`, `TestERC20=0xDDDfdd3D…1dC1`. Required because KH doesn't support chain 16602 (0G Galileo) and wants a chain it can drive `web3/read-contract`/`web3/write-contract` against. Webhook→release path tested live on this Base Sepolia deployment via §7's KeeperHub e2e. **Post-deploy follow-ups status:** **(c) ✅ DONE LIVE** — deployed `TestERC20=0x89a87b531e37731a77b1e40b6b8b5bcb58819059` ("aUSD"), wrote `scripts/e2e-escrow-release.ts` which (1) approves + locks 1 aUSD into Escrow with `orderRefId="order-e2e-…"` and 1% native bond, (2) boots Hono receiver on `:4101` with `skipPin:false, skipRelease:false`, (3) BankSim fires signed PSP webhook, (4) receiver HMAC-verifies → pins to 0G Storage (root `0x31c39254…7f03`, txSeq 49043) → calls `Escrow.release(keccak256(orderRefId), rootHash)` (deployer = keeper), (5) on-chain `Released` event observed with matching evidenceHash, order state = RELEASED, escrow balance = 0. **Bug found + fixed during this run:** `webhook.ts` was double-hashing the evidenceHash (`ethers.id(rootHash)`) before passing to release(), so on-chain hash diverged from the actual 0G root — fixed to pass the rootHash through as bytes32. **(a) and (b) partially done:** root cause for 137 was **macOS Gatekeeper quarantining the unsigned `kh` binary** (NOT a TTY issue) — fixed with `xattr -d com.apple.quarantine /opt/homebrew/Caskroom/kh/0.9.0/kh`. Authenticated via `KH_API_KEY` env var (no interactive login needed). Both workflow shells created on KeeperHub: `tej561683650z2w8d1q1b` (Aegis: Order Monitor & Slash) and `kap46a2jtab0dex082u0y` (Aegis: Payment Verify Webhook Forwarder), both paused. **Remaining (dashboard work, human-only) — informed by actually reading docs.keeperhub.com:** Real KeeperHub model has Triggers (Scheduled/Webhook/Event/Manual), Actions (Web3, Code, Notification, Webhook plugins), Conditions — but **no `for-each` node** (the BUILD.md YAML's for-each is fictional). No `kh secret` CLI either — secrets/connections are dashboard-only. Wiring plan: **§6 monitor (`tej561683650z2w8d1q1b`)** — Scheduled trigger → `web3/read-contract` `getLockedOrders()` on `0x31da…e228` → **Code plugin** (JS loop over the returned `uint256[]`, each iteration reads `getOrderDeadline()` and calls `expire(orderId)` if past deadline) → Discord notify. **§7 forwarder (`kap46a2jtab0dex082u0y`)** — Webhook trigger (auto-generates URL → put into `PAYMENT_WEBHOOK_URL` in `.env`) → `Send Webhook` action POSTing `http://localhost:4001/webhook/payment` with `X-Webhook-Signature` header passthrough. Both then `kh wf go-live <id>`. PSP dashboard configuration (Razorpay/Venmo/Revolut) remains human-only by definition. |
| 9. Agent Runtime | ✅ Done | `agents/runtime/index.ts` (BaseAgent class), `agents/fiat-agent/index.ts` (FiatAgent with RFQ broadcast, quote selection, escrow release listener), `agents/crypto-agent/index.ts` (CryptoAgent LP with quote generation, inventory check, fiat details encryption). All agents integrate AXL transport, MCP client, x402 payments, and 0G Storage. `scripts/verify-section9.ts` passes; `tsc --noEmit` clean. Full e2e requires AXL node on :9002. |
| 10. Web Application | ✅ Done | Next.js 16 app in `apps/web/`. Main swap UI at `/` with wagmi + injected connector (MetaMask). 4-panel layout: (1) Swap form with approve→lock→pay flow, (2) Order state machine (INIT→LOCKED→RELEASED), (3) Activity log, (4) Transaction links to 0G Explorer. API routes: `/api/pay` (fires BankSim webhook), `/api/register-order` (writes to `/tmp/aegis-orders.json`), `/api/sandbox/ws`. Real on-chain flow on 0G Galileo (16602): ERC20 approve → Escrow.lock → webhook receiver validates + pins to 0G Storage → Escrow.release. `scripts/demo.sh` launches AXL + webhook receiver + Next.js. |
| 11. Verification Checklists | ✅ Done | Full e2e verified: wallet connect → approve → lock → BankSim webhook → HMAC verify → 0G Storage pin → escrow release. All services real except fiat payment (BankSim). |
